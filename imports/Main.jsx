...
export default class Main extends React.Component{
	...
	findAll(){
		...
		Tracker.autorun(()=>{
			incomes = Income.find({project:this.props.location.state.id}).fetch(),
			expenses = Expenses.find({project:this.props.location.state.id}).fetch()
		})
		var totalInc = init;
		var totalExp = 0;
		debugger
	...
	updateIncomes(id, name, amount, category){
		...
		Income.update({_id:id},{$set:{name:name, amount:amount, category:category}},()=>{
				this.findAll()
			})
		} else if(name && amount){
			Income.update({_id:id},{$set:{name:name, amount:amount}},()=>{
				this.findAll()
			})
		} else if(name && category){
			Income.update({_id:id},{$set:{name:name, category:category}},()=>{
				this.findAll()
			})
		} else if(amount && category){
			Income.update({_id:id},{$set:{amount:amount, category:category}},()=>{
				this.findAll()
			})
		} else if(name){
...
getIncomeData(name, amount, category, new_category){
	...
	updateExpenses(id, name, amount, category){
		debugger
		if (name && amount && category){
		Expenses.update({_id:id},{$set:{name:name, amount:amount, category:category}},()=>{
				this.findAll()
			})
		}else if(name){
			...
			Expenses.update({_id:id},{$set:{category:category}},()=>{
				this.findAll()
			})
		}else{
			return
...
className="ui inverted grey button"
	onClick={this.redirect.bind(this)}
  	...
  	><i className="backward icon"></i>back</button>
		...
		{/*		<Form getIncomeData = {this.getIncomeData}/>*/}
				{this.state.income_form_open && <Form getIncomeData = {this.getIncomeData} categoriesInc={this.state.categoriesInc} />}
				{this.state.expense_form_open && <Forma getExpensesData = {this.getExpensesData} categories={this.state.categories} />}
...
// 	updateExpenses(id, name, amount){
// 		Expenses.update({_id:id},{$set:{
// 			name:name,
// 			amount:amount
// 		}})
// 		this.findAll()
...
// 		debugger
// 		if (name && amount){
// 		Expenses.update({_id:id},{$set:{name:name, amount:amount}},()=>{
// 				this.findAll()
// 			})
// 		}else if(name){
// 			Expenses.update({_id:id},{$set:{name:name}},()=>{
...
//   				className="mini ui button ui inverted red button"
//   				><i class="minus circle icon"></i>Expense</button>
// 			</div>
// 				<Form getIncomeData = {this.getIncomeData}/>
// 				{this.state.expense_form_open && <Forma getExpensesData = {this.getExpensesData} categories={this.state.categories} />}
// 				<div className="ui inverted divider"></div>
